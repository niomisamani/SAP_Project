...
var queueitems = 0; // initially it's 0
...
function setupparkingmanager() {
    ...
    // Call the updateBulbColor function initially
    updateBulbColor();
}

function updateBulbColor() {
    for (let i = 0; i < parklist.length; i++) {
        const carIsPresent = parklist[i] === 1;
        const bulb = document.querySelector(`.bulb${i + 1}`);
...
function carexit(slot) {
    if (!parklock) {
        ...
        setTimeout(function () {
            ...
            parklock = false;
            updateBulbColor(); // Call updateBulbColor after the car has left
            queuecheck(slot);
        }, 2000)
    }
}
...
function carenter(slot) {
    if (!document.getElementById('car' + (slot).toString()) && !parklock) {
        ...
        setTimeout(function () {
            parklock = false;
            updateBulbColor(); // Call updateBulbColor after the car has entered
        }, 2000)
    } else {
        carexit(slot)
    }
...
// Call setupparkingmanager to initialize everything
setupparkingmanager();
